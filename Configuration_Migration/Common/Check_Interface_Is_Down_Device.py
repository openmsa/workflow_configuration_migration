...
'''
def is_interface_shutdown(context, device, ifce_name):
    is_shutdown = False
    if ifce_name:
        #push configuration to device.
        data = dict(configuration="do show interface " + ifce_name)

        device.push_configuration(json.dumps(data))
        response = json.loads(device.content)

        #get asynchronous push config status
        context.update(device_push_conf_ret_down=response)
        response = self_device_push_conf_status_ret(device, 300)

        #the status should be down
        status = response.get('status')
        context.update(device_push_conf_end_reponse_down=response)
        if status == constants.FAILED:
            MSA_API.task_error('No push config response.', context, True)

        return_message = response.get('message')
        #return_message = GigabitEthernet1 is up, line protocol is up
        if return_message != None:
            interface_up = re.search(ifce_name+'\s+is up,', return_message, re.IGNORECASE)
            if interface_up:
               is_shutdown = False
            else:
               is_shutdown = True
...
if service_policy:
  for rule in service_policy:
    ...
    if good_values.get(interface_name) == None and bad_values.get(interface_name) == None :
      ...
      is_status_shutdown = is_interface_shutdown(context, device, interface_name)
...
if (len(bad_values)):
  ...
  if (len(bad_values)):
    MSA_API.task_success('The interfaces ('+bad_values_string+') are "UP" on the device, but interfaces ('+good_values_string+') are "DOWN"', context, True)
  else:
    MSA_API.task_success('The interfaces ('+ bad_values_string +') are "UP" on the device', context, True)
else: 
  MSA_API.task_success('Good, Interfaces ('+good_values_string+') are all "DOWN" on the device ', context, True)
